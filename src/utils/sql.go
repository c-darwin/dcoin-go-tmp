...
func (db *DCDB) TxParser(data, hash []byte, myTx bool) (error) {
...
func (db *DCDB) ClearIncompatibleTxSql(whereType interface {},  userId int64, waitError *string) {
	...
	if err != nil {
		waitError = fmt.Sptrintf("%v", ErrInfo(err))
	}
	if num > 0 {
...
func (db *DCDB) RollbackIncompatibleTx(typesArr []string) error {
	...
	whereType = whereType[:len(whereType)-1]
	transactions, err := db.GetList(`SELECT data FROM transactions WHERE type IN (`+whereType+`) AND verified=1 AND used = 0`).String()
	for _, txData := range transactions {
		...
		p.DCDB = db
		p.BinaryData = EncodeLengthPlusData([]byte(txData))
		err = p.ParseDataRollback()
		if err != nil {
			return ErrInfo(err)
		}
		// Удаляем уже записанные тр-ии.
		err = db.ExecSql("DELETE FROM transactions WHERE hash = [hex]", md5)
		if err != nil {
			return ErrInfo(err)
		}
		/*
		 * создает проблемы для tesblock_is_ready
		err = db.ExecSql("DELETE FROM transactions_testblock WHERE hash = [hex]", md5)
		if err != nil {
		...
		*/
		...
		err = db.ExecSql("DELETE FROM queue_tx  WHERE hash = [hex]", md5)
		if err != nil {
			return ErrInfo(err)
		}
		err = db.ExecSql("INSERT INTO queue_tx (hash, data) VALUES ([hex], [hex])", md5, BinToHex(txData))
		if err != nil {
			return ErrInfo(err)
		}
	}
...
func GetTxTypeAndUserId(binaryBlock []byte) (int64, int64, string) {
	...
	if InSliceInt64(txType, TypesToIds([]string{"AdminChangePrimaryKey", "ChangeKeyRequest", "CfProjectData", "CfComment", "CfProjectChangeCategory", "CfSendDc", "DelCfProject", "CashRequestOut", "VotesGeolocation", "VotesMiner", "VotesNodeNewMiner", "VotesPct", "VotesPromisedAmount", "DelPromisedAmount"})) {
		thirdVar = BytesShift(&binaryBlock, DecodeLength(&binaryBlock))
	}
	return txType, userId, thirdVar

}

func (db *DCDB) ClearIncompatibleTx(binaryTx []byte, myTx bool) (string, string, int64, int64, int64, string) {

	var fatalError, waitError string
	var toUserId int64

	// получим тип тр-ии и юзера
	txType, userId, thirdVar := GetTxTypeAndUserId(binaryTx)

	if !CheckInputData(txType, "int")  {
		fatalError = "error type"
	}
	if !CheckInputData(userId, "int")  {
		fatalError = "error userId"
	}
	if !CheckInputData(thirdVar, "int")  {
		fatalError = "error thirdVar"
	}

	if txType == TypeInt("CashRequestOut") {
		toUserId = thirdVar
	}
	var forSelfUse int64
	if InSliceInt64(txType, TypesToIds([]string{"NewPct", "NewReduction", "NewMaxPromisedAmounts", "NewMaxOtherCurrencies"})) {
	...
	if len(fatalError) == 0 {
		db.ClearIncompatibleTxSql(txType, userId, &waitError)
		...
		if InSliceInt64(txType, TypesToIds([]string{"DelPromisedAmount", "ChangePromisedAmount"})) {
			num, err := db.Single(`
						SELECT count(*)
			          	FROM (
				            SELECT user_id
						...
						`, userId, TypeInt("CashRequestIn"), userId, userId, TypeInt("CashRequestIn"), userId).Int64()
			if err != nil {
				fatalError = fmt.Sprintf("%s", err)
			}
			if num > 0 {
				fatalError = "thirdVar = "+Int64ToStr(userId)
			}
		}
		...
		if txType == TypeInt("CashRequestOut") {
			txData, err := db.Single(`
					 ...
					 `, TypeInt("DelPromisedAmount"), TypeInt("ChangePromisedAmount"), toUserId, TypeInt("DelPromisedAmount"), TypeInt("ChangePromisedAmount"), toUserId).Int64()
			if err != nil {
				fatalError = fmt.Sprintf("%s", err)
			}
			if len(txData) > 0 {
				// откатим фронтальные записи
				p := new(dcparser.Parser)
				p.BinaryData = EncodeLengthPlusData(txData)
				p.ParseDataRollback()
				// Удаляем именно уже записанную тр-ию. При этом новая (CashRequestOut) тр-ия успешно обработается
				err = db.ExecSql("DELETE FROM transactions WHERE hash = [hex]", Md5(txData))
				if err != nil {
					fatalError = fmt.Sprintf("%s", err)
				}
				/*
				 * создает проблемы для tesblock_is_ready
				err = db.ExecSql("DELETE FROM transactions_testblock WHERE hash = [hex]md5(?tx_data)?")
				if err != nil {
					db.PrintSleep(err, 60)
					continue BEGIN
				}
		...
		if txType == TypeInt("CashRequestOut") {
			db.ClearIncompatibleTxSqlSet([]string{"SendDc", "NewForexOrder", "CfSendDc"}, userId, &waitError, "")
		}
		// и наоборот
		if InSliceInt64(txType, TypesToIds([]string{"SendDc", "NewForexOrder", "CfSendDc"})) {
			db.ClearIncompatibleTxSqlSet([]string{"CashRequestOut"}, userId, &waitError, "")
		}

		// на всякий случай не даем попасть в один блок holidays и тр-им, где holidays используются
		if txType == TypeInt("NewHolidays") {
			db.ClearIncompatibleTxSql("Mining", userId, &waitError);
		}
		if txType == TypeInt("Mining") {
			db.ClearIncompatibleTxSql("NewHolidays", userId, &waitError);
		}


		if txType == TypeInt("NewHolidays") {
			db.ClearIncompatibleTxSql("CashRequestIn", userId, &waitError);
		}
		if txType == TypeInt("CashRequestIn") {
			db.ClearIncompatibleTxSql("NewHolidays", userId, &waitError);
		}

		if txType == TypeInt("CashRequestOut") {
			db.ClearIncompatibleTxSql("NewHolidays", toUserId, &waitError);
		}

		// не должно попадать в одни блок NewMinerUpdate и NewMiner
		if txType == TypeInt("NewMiner") {
			db.ClearIncompatibleTxSql("NewMinerUpdate", userId, &waitError);
		}
		if txType == TypeInt("NewMinerUpdate") {
			db.ClearIncompatibleTxSql("NewMiner", userId, &waitError);
		}

		// не должно попадать в один блок смена нодовского ключа и тр-ии которые этим ключем подписываются
		if txType == TypeInt("ChangeNodeKey") || txType == TypeInt("NewMiner") {
			db.ClearIncompatibleTxSql("NewMinerUpdate", userId, &waitError);
		}
		if txType == TypeInt("ChangeNodeKey") || txType == TypeInt("NewMiner") {
			db.ClearIncompatibleTxSql("NewPct", userId, &waitError);
		}
		if txType == TypeInt("NewMinerUpdate") {
			db.ClearIncompatibleTxSqlSet([]string{"ChangeNodeKey", "NewMiner"}, userId, &waitError, "");
		}
		if txType == TypeInt("NewPct") {
			db.ClearIncompatibleTxSqlSet([]string{"ChangeNodeKey", "NewMiner"}, userId, &waitError, "");
		}
		if txType == TypeInt("ChangeNodeKey") || txType == TypeInt("NewMiner") {
			db.ClearIncompatibleTxSql("NewReduction", userId, &waitError);
		}

		// восстановление ключа
		if txType == TypeInt("ChangeKeyRequest") {
			db.ClearIncompatibleTxSqlSet([]string{"ChangeKeyActive"}, thirdVar, &waitError, "");
		}
		if txType == TypeInt("ChangeKeyActive") {
			db.ClearIncompatibleTxSqlSet([]string{"ChangeKeyRequest"}, 0, &waitError, userId);
		}

		// нельзя удалить/изменить обещанную сумму и затем создать запрос на её майнинг
		if txType == TypeInt("Mining") {
			db.ClearIncompatibleTxSqlSet([]string{"DelPromisedAmount", "ChangePromisedAmount"}, userId, &waitError, "");
		}
		if InSliceInt64(txType, TypesToIds([]string{"DelPromisedAmount", "ChangePromisedAmount"})) {
			db.ClearIncompatibleTxSql("Mining", userId, &waitError);
		}
		...
		if txType == TypeInt("Mining") {
			db.ClearIncompatibleTxSql("AdminBanMiners", 0, &waitError);
		}

		if txType == TypeInt("CashRequestOut") {
			db.ClearIncompatibleTxSql("AdminBanMiners", 0, &waitError);
		}
		if txType == TypeInt("NewPromisedAmount") {
			db.ClearIncompatibleTxSql("AdminBanMiners", 0, &waitError);
		}

		if txType == TypeInt("AdminBanMiners") {
			db.RollbackIncompatibleTx([]string{"CashRequestOut", "change_host", "NewPromisedAmount", "ChangeNodeKey", "NewPct", "Mining", "VotesMiner", "VotesNodeNewMiner", "VotesPromisedAmount", "Abuses", "NewPromisedAmount", "VotesComplex"}, 0, &waitError)
		}


		if txType == TypeInt("VotesMiner") {
			db.ClearIncompatibleTxSqlSet([]string{"AdminBanMiners"}, 0, &waitError, "");
		}
		if txType == TypeInt("VotesComplex") {
			db.ClearIncompatibleTxSqlSet([]string{"AdminBanMiners"}, 0, &waitError, "");
		}
		if txType == TypeInt("Abuses") { // AdminBanMiners преоритетнее, abuses надо вытеснять
			db.ClearIncompatibleTxSqlSet([]string{"AdminBanMiners"}, 0, &waitError, ""); // дополнить
		}
		if txType == TypeInt("VotesNodeNewMiner") {
			db.ClearIncompatibleTxSqlSet([]string{"AdminBanMiners"}, 0, &waitError, "");
		}
		if txType == TypeInt("VotesPromisedAmount") {
			db.ClearIncompatibleTxSqlSet([]string{"AdminBanMiners"}, 0, &waitError, "");
		}
		...
		if txType == TypeInt("VotesPromisedAmount") {
			promisedAmountUserId, err := db.Single("SELECT user_id FROM promised_amount WHERE id  =  ?", thirdVar).Int64()
			if err != nil {
				fatalError = fmt.Sprintf("%s", err)
			}
			if promisedAmountUserId > 0 {
				db.ClearIncompatibleTxSqlSet([]string{"ChangeGeolocation"}, promisedAmountUserId, &waitError, "");
			}
		}

		// нельзя менять местоположение, если кто-то отдал голос за мою обещанную сумму
		if txType == TypeInt("ChangeGeolocation") {
			promisedAmountIds_, err := db.GetList("SELECT id FROM promised_amount WHERE user_id  = ?", userId).String()
			if err != nil {
				fatalError = fmt.Sprintf("%s", err)
			}
			promisedAmountIds := strings.Join(promisedAmountIds_, ",")
			if len(promisedAmountIds) {
				num, err := db.Single(`
							SELECT count(*)
						     FROM (
						        SELECT user_id
							...
							`, TypeInt("VotesPromisedAmount"), TypeInt("VotesPromisedAmount")).Int64()
				if err != nil {
					fatalError = fmt.Sprintf("%s", err)
				}
				if num > 0 {
		...
		if txType == TypeInt("DelCfProject") {
			db.ClearIncompatibleTxSqlSet([]string{"CfComment","CfSendDc","CfProjectChangeCategory","CfProjectData"}, 0, &waitError, thirdVar);
		}
		if InSliceInt64(txType, TypesToIds([]string{"CfComment", "CfSendDc", "CfProjectChangeCategory", "CfProjectData"})) {
			db.ClearIncompatibleTxSqlSet([]string{"DelCfProject"}, 0, &waitError, thirdVar);
		}

		// потом нужно сделать более тонко. но пока так. Если есть удаление проекта, тогда откатываем все тр-ии del_cf_funding
		if txType == TypeInt("DelCfProject") {
			db.RollbackIncompatibleTx([]string{"DelCfFunding"});
		}
		// потом нужно сделать более тонко. но пока так. Если есть del_cf_funding, тогда откатываем все тр-ии удаления проектов
		if txType == TypeInt("DelCfFunding") {
			db.RollbackIncompatibleTx([]string{"DelCfProject"});
		}
		// потом нужно сделать более тонко. но пока так. Если есть смена комиссии, то нельзя отправлять тр-ии, где указана комиссия
		if InSliceInt64(txType, TypesToIds([]string{"CfSendDc", "SendDc", "NewForexOrder"})) {
			db.RollbackIncompatibleTx( array("ChangeCommission") );
		}
		if txType == TypeInt("ChangeCommission") {
			db.ClearIncompatibleTxSqlSet([]string{"CfSendDc", "SendDc", "NewForexOrder"}, 0, &waitError );
		}


		// Если есть смена коммиссий арбитров, то нельзя делать перевод монет, т.к. там может быть указана комиссия арбитра
		if InSliceInt64(txType, TypesToIds([]string{"SendDc"})) {
			db.RollbackIncompatibleTx( []string{"ChangeArbitratorConditions"} );
		}
		if txType == TypeInt("ChangeArbitratorConditions") {
			db.ClearIncompatibleTxSqlSet( []string{"SendDc"}, 0, &waitError, "");
		}
		// если идет смена списка арбитров, то у отправителя и у получателя может получиться нестыковка
		if InSliceInt64(txType, TypesToIds([]string{"SendDc"})) {
			db.RollbackIncompatibleTx( []string{"ChangeArbitratorList"} );
		}
		if txType == TypeInt("ChangeArbitratorList") {
			db.ClearIncompatibleTxSqlSet( []string{"SendDc"}, 0, &waitError, "");
		}
		...
		if txType == TypeInt("CfSendDc") {
			db.ClearIncompatibleTxSqlSet( []string{"CfSendDc","CfComment","DelCfProject","CfProjectChangeCategory","CfProjectData"}, 0, &waitError, thirdVar);
		}
		if InSliceInt64(txType, TypesToIds([]string{"CfSendDc", "CfComment", "DelCfProject", "CfProjectChangeCategory", "CfProjectData"})){
			db.ClearIncompatibleTxSqlSet( []string{"CfSendDc"}, 0, &waitError, thirdVar);
		}


		// нельзя удалять promised_amount и голосовать за него
		if txType == TypeInt("DelPromisedAmount") {
			db.ClearIncompatibleTxSqlSet( []string{"VotesPromisedAmount"}, 0, &waitError, thirdVar);
		}
		if txType == TypeInt("VotesPromisedAmount") {
			db.ClearIncompatibleTxSqlSet( []string{"DelPromisedAmount"}, 0, &waitError, thirdVar);
		}
		if txType == TypeInt("NewMaxPromisedAmounts") {
			db.ClearIncompatibleTxSqlSet( []string{"NewMaxPromisedAmounts"}, 0, &waitError, thirdVar);
		}
		if txType == TypeInt("NewMaxOtherCurrencies") {
			db.ClearIncompatibleTxSqlSet( []string{"NewMaxOtherCurrencies"}, 0, &waitError, thirdVar);
		}
		if txType == TypeInt("NewPct") {
			db.ClearIncompatibleTxSqlSet( []string{"NewPct"}, 0, &waitError, thirdVar);
		}
		if txType == TypeInt("NewReductions") {
			db.ClearIncompatibleTxSqlSet( []string{"NewReductions"}, 0, &waitError, thirdVar);
		}

		// в один блок должен попасть только один голос за один объект голосования. thirdVar - объект голосования
		if InSliceInt64(txType, TypesToIds([]string{"VotesPromisedAmount", "VotesMiner", "VotesNodeNewMiner", "VotesComplex"})) {
			num, err := db.Single(`
			  			  SELECT count(*)
				            FROM (
					            SELECT user_id
							...
							`, TypeInt("VotesPromisedAmount"), TypeInt("VotesMiner"), TypeInt("VotesNodeNewMiner"), TypeInt("VotesComplex"), thirdVar, TypeInt("VotesPromisedAmount"), TypeInt("VotesMiner"), TypeInt("VotesNodeNewMiner"), TypeInt("VotesComplex"), thirdVar).Int64()
			if err != nil {
				fatalError = fmt.Sprintf("%s", err)
			}
			if num > 0 {
		...
		if txType == TypeInt("CashRequestIn") {
			txData, err := db.Single(`
					  SELECT *
						...
						`, TypeInt("DelPromisedAmount"), TypeInt("ChangePromisedAmount"), userId, TypeInt("DelPromisedAmount"), TypeInt("ChangePromisedAmount"), userId).Int64()
			if err != nil {
				fatalError = fmt.Sprintf("%s", err)
			}
			if len(txData) > 0 {
				...
				/*
				 * создает проблемы для tesblock_is_ready
				err = db.ExecSql("DELETE FROM transactions_testblock WHERE hash = [hex]", Md5(txData))
				if err != nil {
					db.PrintSleep(err, 60)
					continue BEGIN
				}
		...
		if txType == TypeInt("ChangePrimaryKey") {
			num, err := db.Single(`
						  SELECT count(*)
				            FROM (
					            SELECT user_id
		...
		if txType == TypeInt("AdminChangePrimaryKey") {
			num, err := db.Single(`
						  SELECT count(*)
				            FROM (
					            SELECT user_id
		...
		num, err := db.Single(`
						...
						`, TypeInt("ChangePrimaryKey"), userId, TypeInt("NewPct"), TypeInt("NewReduction"), TypeInt("ChangePrimaryKey"), userId, TypeInt("NewPct"), TypeInt("NewReduction")).Int64()
		if err != nil {
			fatalError = fmt.Sprintf("%s", err)
		}
		if num > 0 {
		...
		if txType == TypeInt("NewPct") {
			db.RollbackIncompatibleTx( []string{"NewReduction", "ChangeNodeKey", "NewMiner", "VotesPromisedAmount", "SendDc", "CashRequestIn", "Mining", "CfSendDc", "DelCfProject", "NewForexOrder", "del_forex_order", "for_repaid_fix", "actualization_promised_amounts", "DelCfFunding", "admin_unban_miners", "AdminBanMiners"} );
		}

		// если пришло NewReduction, то нужно откатить следующие тр-ии
		if txType == TypeInt("NewReduction") {
			db.RollbackIncompatibleTx([]string{"NewPct", "ChangeNodeKey", "NewMiner", "VotesPromisedAmount", "SendDc", "CashRequestIn", "Mining", "CfSendDc", "DelCfProject", "NewForexOrder", "del_forex_order", "for_repaid_fix", "actualization_promised_amounts", "DelCfFunding", "admin_unban_miners", "AdminBanMiners"});
		}
