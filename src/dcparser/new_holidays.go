...
func (p *Parser) NewHolidays() (error) {
	...
	//var myUserIds []int64;
	err := p.ExecSql(`INSERT INTO holidays (user_id,	start_time,end_time) VALUES ($1, $2, $3)`,
	...
	if utils.BytesToInt64(p.TxMap["user_id"]) == myUserId && myBlockId <= p.BlockData.BlockId {
		...
		err := p.ExecSql("DELETE FROM "+myPrefix+"my_holidays WHERE start_time=$1 AND end_time=$2", p.TxMap["start_time"], p.TxMap["end_time"])
		if err != nil {
			return err
		}
	}
...
func (p *Parser) NewHolidaysRollback() (error) {
	//fmt.Println(p.TxMap)
	err := p.ExecSql("DELETE FROM holidays WHERE user_id=$1 AND start_time=$2 AND end_time=$3", p.TxMap["user_id"], p.TxMap["start_time"], p.TxMap["end_time"])
