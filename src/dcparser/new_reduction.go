...
func (p *Parser) NewReduction() (error) {
	...
	if utils.StrToFloat64(p.TxMaps.String["pct"]) > 0 {
		...
		err := p.ExecSql("UPDATE wallets SET amount_backup = amount, amount = amount*("+utils.Float64ToStr(d)+") WHERE currency_id = ?", p.TxMaps.Int64["currency_id"])
		if err != nil {
			return p.ErrInfo(err)
		}
		...
		err = p.ExecSql("UPDATE promised_amount SET tdc_amount_backup = tdc_amount, tdc_amount = tdc_amount*("+utils.Float64ToStr(d)+") WHERE currency_id = ?", p.TxMaps.Int64["currency_id"])
		if err != nil {
			return p.ErrInfo(err)
		}
		...
		// форeкс-ордеры
		err = p.ExecSql("UPDATE forex_orders SET amount_backup = amount, amount = amount*("+utils.Float64ToStr(d)+") WHERE sell_currency_id = ?", p.TxMaps.Int64["currency_id"])
		if err != nil {
			return p.ErrInfo(err)
		}

		// крауд-фандинг
		err = p.ExecSql("UPDATE cf_funding SET amount_backup = amount, amount = amount*("+utils.Float64ToStr(d)+") WHERE currency_id = ?", p.TxMaps.Int64["currency_id"])
		if err != nil {
			return p.ErrInfo(err)
		}
	}
