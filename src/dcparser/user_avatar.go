...
func (p *Parser) UserAvatar() (error) {
	return p.selectiveLoggingAndUpd([]string{"host", "avatar"}, []string{p.TxMaps.String["host"], p.TxMaps.String["avatar"]}, "users", []string{"user_id"}, []string{utils.Int64ToStr(p.TxUserID)})
}
