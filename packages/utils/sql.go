...
func (db *DCDB) checkAvailableKey(key string) (int64, string, error) {
	publicKeyAsn, err := GetPublicFromPrivate(key)
	if err != nil {
		log.Debug("%v", err)
		return 0, "", ErrInfo(err)
	}
	...
	if InSliceString(Int64ToStr(userId)+"_my_table", allTables) {
		return 0, "", errors.New("exists _my_table")
	}
	return userId, string(publicKeyAsn), nil
}

func (db *DCDB) GetAvailableKey() (int64, string, error) {
	keysStr, err := GetHttpTextAnswer("http://dcoin.club/keys")
	if err != nil {
		return 0, "", ErrInfo(err)
	}
	...
	for _, key := range keys {
		userId, pubKey, err := db.checkAvailableKey(key)
		if err != nil {
			log.Error("%s", ErrInfo(err)) // тут ошибка - это нормально
		}
		...
		if userId > 0 {
			...
			myPref := ""
			if len(community) > 0 {
				myPref = Int64ToStr(userId)+"_"
				err = db.ExecSql("INSERT INTO "+myPref+"my_table (user_id, status) VALUES (?, ?)", userId, "waiting_set_new_key")
				if err != nil {
					return 0, "", ErrInfo(err)
				}
			} else {
				err = db.ExecSql("UPDATE my_table SET user_id = ?, status = ?", userId, "waiting_set_new_key")
				if err != nil {
					return 0, "", ErrInfo(err)
				}
			}
			...
			err = db.ExecSql("INSERT INTO "+myPref+"my_keys (private_key, public_key, status, block_id) VALUES (?, [hex], ?, ?)", key, pubKey, "approved", 1)
			if err != nil {
				return 0, "", ErrInfo(err)
			}
			...
			err = db.ExecSql("INSERT INTO "+myPref+"my_keys (private_key, public_key, status) VALUES (?, ?, ?)", newPrivKey, HexToBin([]byte(newPubKey)), "my_pending")
			if err != nil {
				return 0, "", ErrInfo(err)
			}
