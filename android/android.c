...
void ANativeActivity_onCreate(ANativeActivity *activity, void* savedState, size_t savedStateSize) {
	...
	if (!main_running) {
		...
		jmethodID getfiledir = find_method(env, current_ctx_clazz, "getFilesdir", "()Ljava/lang/String;");
		jstring path = (jstring)(*env)->CallObjectMethod(env, current_ctx, getfiledir, NULL);
		const char* filesdir = (*env)->GetStringUTFChars(env, path, NULL);
		if (setenv("FILESDIR", filesdir, 1) != 0) {
			LOG_INFO("setenv(\"FILESDIR\", \"%s\", 1) failed: %d", filesdir, errno);
		}
		(*env)->ReleaseStringUTFChars(env, path, filesdir);
