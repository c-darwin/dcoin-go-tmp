...
import (
	...
	"github.com/c-darwin/dcoin-go-tmp/packages/notif"
)
...
var Gactivity *C.ANativeActivity
//export onCreate
func onCreate(activity *C.ANativeActivity) {
  
    notif.InitNotif(unsafe.Pointer(activity));

    
 // C.NotifTest()
  
  fmt.Println("internalDataPath ", activity.internalDataPath)
  fmt.Println("internalDataPath ", C.GoString(activity.internalDataPath))
  C.setenv(C.CString("FILESDIR"), activity.internalDataPath, 1)
